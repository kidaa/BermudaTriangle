...
class INFO {
	...
	public:
		...
		VOID SetSound( BOOL s ) { fSound = s; }
		BOOL GetSound() { return fSound; }
		VOID SetSoundAvailability( BOOL f ) { SoundAvailable = f;
											if( !SoundAvailable ) fSound = FALSE; }
		BOOL IsSoundAvailable() { return SoundAvailable; }
		char ReturnLastHigh() { return ScoreList[9].Score; }	
		VOID InsertHigh( char *n, char s );
		INFO() { fSound = FALSE;
					LineStyle = IDC_DIAGONALS_AND_VERTICALS; }
