...
//	WinBeginPaint( pg->hwndClient, hpsGlob, &rectl );
	...
	if( !RealPaint ){
	    POINTL aptl[3];
	    aptl[0].x = aptl[2].x = rectl.xLeft;
	    aptl[0].y = aptl[2].y = rectl.yBottom;
...
//		GBoard.ShowPointerPos( hwndToAck, 0, 0 ); // removes the square
	    ...
	    WinFillRect( hpsGlob, &rectl, CLR_WHITE );	// clear background
...
//    	return (MRESULT) 0;
    ...
    case WM_DRAWPMMARK:
	GBoard.DrawPMMark( hpsGlob, (CHAR)SHORT1FROMMP( mp2 ),
			   (CHAR)SHORT2FROMMP( mp2 ) );
	GBoard.DrawPMMark( hpsBufferGlob, (CHAR)SHORT1FROMMP( mp2 ),
			   (CHAR)SHORT2FROMMP( mp2 ) );
	break;
	...
	GBoard.MarkDragLine( hpsGlob, (CHAR)SHORT1FROMMP(mp1),
			     (CHAR)SHORT2FROMMP(mp1) );
	GBoard.MarkDragLine( hpsBufferGlob, (CHAR)SHORT1FROMMP(mp1),
			     (CHAR)SHORT2FROMMP(mp1) );
	break;
