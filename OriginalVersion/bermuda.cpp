...
MRESULT EXPENTRY HighScoreDlgProc( HWND hwnd, ULONG msg, MPARAM mp1, 
				   MPARAM mp2 );
MRESULT EXPENTRY ProdInfoDlgProc( HWND hwnd, ULONG msg, MPARAM mp1, 
				  MPARAM mp2 );
MRESULT EXPENTRY GameSettingsDlgProc( HWND hwnd, ULONG msg, MPARAM mp1, 
				      MPARAM mp2 );
MRESULT EXPENTRY ShowHighDlgProc( HWND hwnd, ULONG msg, MPARAM mp1, 
				  MPARAM mp2 );
...
VOID WriteProfile( HWND hwndFrame );
VOID ReadProfile( HWND hwndFrame );
...
main( /* INT argc, CHAR *argv[], CHAR *envp[] */ )
{
    ...
    Sound::SetSoundWanted( TRUE );

    
    Profile = new PROFILE( hab, pszAppName, pszIniName, pszVersion );
    
    {
	    // necessary because sound must be set before the window creation
	BOOL test;
	Profile->Open();
	BOOL fSndTmp = Sound::GetSoundWanted();
	wcprintf("Pos. 1");
	if( fSndTmp )
	    wcprintf("Sound ist an");
	else
	    wcprintf("Sound ist aus");
	ULONG lNumRead = sizeof( BOOL );
	test = Profile->ReadData( pszAppName, PrfKeys.pszSound, &fSndTmp, 
				  &lNumRead );
	wcprintf("Erfolg: %d", test);
	Sound::SetSoundWanted( fSndTmp );
	wcprintf("Pos. 2");
	if( fSndTmp )
	    wcprintf("Sound ist an");
	else
...
//    WinSetWindowULong( hwnd, QWL_USER, (ULONG) pg );
    ...
    case WM_HITTEST:   // check whether we let the mouse pointer through
	{
	    RECTL rectl;
	    WinQueryWindowRect( hwnd, &rectl );
	    POINTS pts = { SHORT1FROMMP( mp1 ), SHORT2FROMMP( mp1 ) };
	    LONG xMiddle = rectl.xLeft + (rectl.xRight - rectl.xLeft) / 2;
	    
	    if( pts.x < xMiddle ){
		if( pts.y < 
		    (- pts.x * rectl.yTop / xMiddle + rectl.yTop) )
		    return MRFROMLONG(HT_TRANSPARENT);  // the window is not there
	    } else {
		if( pts.y < 
		    (pts.x * rectl.yTop / xMiddle - rectl.yTop ) )
		    return MRFROMLONG(HT_TRANSPARENT);  // the window is not there
	    ...
	    // if this didn't help:
	    return MRFROMLONG(HT_NORMAL);       // act as usual
	}
...
//			WinSendMsg( pg->hwndTSound, WM_DESTROY, mp1, mp2 );
	...
	WriteProfile( WinQueryWindow( hwnd, QW_PARENT ) );
	delete pg;
	return (MRESULT) 0;
...
//	return (MRESULT) TRUE;
	...
	{
	    RECTL rectl;
//	    WinBeginPaint( hwnd, hpsGlob, &rectl );
	    ...
	    WinSendMsg( pg->hwndTGraph, WM_SHOWPOINTERPOS, MPFROMHWND(hwnd),
			MPFROM2SHORT( 0, 0 ) );	
	    // war Post:
	    WinSendMsg( pg->hwndTGraph, WM_USER_PAINT, MPFROMHWND(hwnd), 
			MPFROMLONG( RealPaint ) );
	    RealPaint = FALSE;     // reset the painting flag
//	WinEndPaint( hpsGlob );
//			return (MRESULT)0;
	}
	break;
...
//			GBoard.ShowPointerPos( hwnd, ptl.x, ptl.y ); // redisplay ptr square
	...
	case HT_NORMAL:
	    wcprintf("HT_NORMAL"); break;
	case HT_TRANSPARENT:
	    wcprintf("HT_TRANSPARENT"); break;
	default:
	    wcprintf("other hittest");
	}
...
MRESULT EXPENTRY GameSettingsDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2 )
{
    ...
    switch( msg ){
    case WM_INITDLG:
	(VOID)WinCheckButton( hwnd, (USHORT)IDC_DIAGONALS, 
			      InfoData.IsLineStyle(IDC_DIAGONALS) );
	(VOID)WinCheckButton( hwnd, (USHORT)IDC_VERTICALS, 
			      InfoData.IsLineStyle(IDC_VERTICALS));
	(VOID)WinCheckButton( hwnd, (USHORT)IDC_DIAGONALS_AND_VERTICALS, 
			      InfoData.IsLineStyle
			      (IDC_DIAGONALS_AND_VERTICALS));
	(VOID)WinCheckButton( hwnd, (USHORT)IDC_SOUND, 
			      Sound::GetSoundWanted() );
	break;
    case WM_COMMAND:
	switch( SHORT1FROMMP( mp1 ) ){
	case DID_OK:
	    ...
	    Sound::SetSoundWanted( WinQueryButtonCheckstate(hwnd, IDC_SOUND) );
...
MRESULT EXPENTRY ShowHighDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2 )
{
    ...
    case WM_INITDLG:
	for( i = 0; i < 10; i++){
	    WinSetDlgItemText( hwnd, ULONG(IDR_HIGHNAME1 + i) , 
			       InfoData.GetHighName(i) );
				// item IDR_HIGHNAME1 ... IDR_HIGHNAME10
	    WinSetDlgItemShort( hwnd, ULONG(IDR_HIGHSCORE1 + i), 
				(USHORT)InfoData.GetHighScore(i),
				FALSE);
...
VOID ReadProfile( HWND hwndFrame )
{
    // here the profile data is read into the variables:
    Profile->Open();

    // retrieve line style settings
    ULONG lNumRead = sizeof( ULONG );
    ULONG tmpLineStyle;
    if( Profile->ReadData( pszAppName, PrfKeys.pszLineStyle,
			   &tmpLineStyle, &lNumRead ) )
...
VOID WriteProfile( HWND hwndFrame )
{
    ...
    // write line style
    ULONG tmpLineStyle = InfoData.GetLineStyle();
    Profile->WriteData( pszAppName, PrfKeys.pszLineStyle,
			(PVOID)&tmpLineStyle, (ULONG)sizeof(ULONG) );
    // write sound flag
    BOOL tmpSound = Sound::GetSoundWanted();
    Profile->WriteData( pszAppName, PrfKeys.pszSound,
			(PVOID)&tmpSound, (ULONG)sizeof(BOOL) );
